...
for frame_id in range(0, total_frames): 
    ...
    for track in tracker.tracks:
        ...
        rectangle_tracking = bbox
        detections_rectangle_vector = [det.to_tlbr() for det in detections_class]
        rectangle_detection = utils.get_closest_rectangle_to(rectangle_tracking,detections_rectangle_vector)
        if rectangle_detection is not None:
            annotation = {
                "track_id": int(id_num),
                "image_id": frame_id + 1,
                "bbox" : [
                    rectangle_detection[0],
                    rectangle_detection[1],
                    rectangle_detection[2] - rectangle_detection[0],
                    rectangle_detection[3] - rectangle_detection[1]
                ],
                "scores" : [],
                "category_id": 1
            } 
            ...
            #Draw bbox from detections
            cv2.rectangle(frame, (int(rectangle_detection[0]), int(rectangle_detection[1])), (int(rectangle_detection[2]), int(rectangle_detection[3])),(255,255,0), 2)
